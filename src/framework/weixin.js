var context = {
  'wx': [{
    ...
    },
    {
      ...
      "info": "将本地资源上传到开发者服务器。如页面通过 wx.chooseImage 等接口获取到一个本地资源的临时文件路径后，可通过此接口将本地资源上传到指定服务器。客户端发起一个 HTTPS POST 请求，其中  为  。"
    },
    {
      "name": "downloadFile",
      "value": "downloadFile({\n\t\n})",
      ...
      "info": "下载文件资源到本地。客户端直接发起一个 HTTP GET 请求，返回文件的本地临时路径。"
    },
    {
      "name": "connectSocket",
      "value": "connectSocket({\n\turl: ''\n})",
    ...
    },
    {
      ...
      "info": "通过 WebSocket 连接发送数据，需要先 wx.connectSocket，并在 wx.onSocketOpen 回调之后才能发送。"
    },
    {
    ...
    },
    {
      ...
      "info": "保存图片到系统相册，需要用户授权（scope.writePhotosAlbum），详见 用户授权"
    },
    {
      "name": "startRecord",
      "value": "startRecord({\n\t\n})",
      ...
      "info": "开始录音。当主动调用，或者录音超过1分钟时自动结束录音，返回录音文件的临时文件路径。当用户离开小程序时，此接口无法调用。"
    },
    {
      "name": "stopRecord",
      "value": "stopRecord()",
    ...
    },
    {
      ...
      "info": "开始播放语音，同时只允许一个语音文件正在播放，如果前一个语音文件还没播放完，将中断前一个语音播放。"
    },
    {
      "name": "pauseVoice",
      "value": "pauseVoice()",
      ...
      "info": "暂停正在播放的语音。再次调用wx.playVoice播放同一个文件时，会从暂停处开始播放。如果想从头开始播放，需要先调用 wx.stopVoice。"
    },
    {
      "name": "stopVoice",
      "value": "stopVoice()",
    ...
    },
    {
      ...
      "info": "使用后台播放器播放音乐，对于微信客户端来说，只能同时有一个后台音乐在播放。当用户离开小程序后，音乐将暂停播放；当用户点击“显示在聊天顶部”时，音乐不会暂停播放；当用户在其他小程序占用了音乐播放器，原有小程序内的音乐将停止播放。"
    },
    {
    ...
    },
    {
      ...
      "info": "创建并返回 audio 上下文 audioContext 对象"
    },
    {
      "name": "chooseVideo",
      "value": "chooseVideo({\n\t\n})",
      ...
      "info": "拍摄视频或从手机相册中选视频，返回视频的临时文件路径。"
    },
    {
      ...
      "info": "保存视频到系统相册，需要用户授权（scope.writePhotosAlbum），详见 用户授权"
    },
    {
      ...
      "info": "创建并返回 video 上下文 videoContext 对象"
    },
    {
      "name": "saveFile",
      "value": "saveFile({\n\ttempFilePath: ''\n})",
    ...
    },
    {
      ...
      "info": "获取本地文件的文件信息。此接口只能用于获取已保存到本地的文件，若需要获取临时文件信息，请使用 wx.getFileInfo 接口。"
    },
    {
    ...
    },
    {
      ...
      "info": "新开页面打开文档，支持格式：doc, xls, ppt, pdf, docx, xlsx, pptx"
    },
    {
      "name": "setStorage",
      "value": "setStorage({\n\tkey: '',\n\tdata: ''\n})",
      ...
      "info": "将数据存储在本地缓存中指定的 key 中，会覆盖掉原来该 key 对应的内容，这是一个异步接口。"
    },
    {
      "name": "setStorageSync",
      ...
      "info": "将 data 存储在本地缓存中指定的 key 中，会覆盖掉原来该 key 对应的内容，这是一个同步接口。"
    },
    {
      "name": "getStorage",
      "value": "getStorage({\n\tkey: '',\n\tsuccess: function (res) {}\n})",
    ...
    },
    {
      ...
      "info": "获取当前的地理位置、速度。当用户离开小程序后，此接口无法调用；当用户点击“显示在聊天顶部”时，此接口可继续调用。"
    },
    {
      "name": "chooseLocation",
    ...
    },
    {
      ...
      "info": "创建并返回 map 上下文 mapContext 对象"
    },
    {
      "name": "getSystemInfo",
      "value": "getSystemInfo({\n\tsuccess: function (res) {}\n})",
    ...
    },
    {
      ...
      "info": "判断小程序的API，回调，参数，组件等是否在当前版本可用。"
    },
    {
      "name": "getNetworkType",
    ...
    },
    {
      ...
      "info": "监听加速度数据，频率：5次/秒，接口调用后会自动开始监听，可使用  停止监听。"
    },
    {
    ...
    },
    {
      ...
      "info": "监听罗盘数据，频率：5次/秒，接口调用后会自动开始监听，可使用停止监听。"
    },
    {
      "name": "startCompass",
      "value": "startCompass({\n\t\n})",
    ...
    },
    {
      ...
      "info": "关闭蓝牙模块。调用该方法将断开所有已建立的链接并释放系统资源"
    },
    {
    ...
    },
    {
      ...
      "info": "开始搜寻附近的蓝牙外围设备。注意，该操作比较耗费系统资源，请在搜索并连接到设备后调用 stop 方法停止搜索。"
    },
    {
      ...
      "info": "停止搜寻附近的蓝牙外围设备。请在确保找到需要连接的设备后调用该方法停止搜索。"
    },
    {
    ...
    },
    {
      ...
      "info": "读取低功耗蓝牙设备的特征值的二进制数据值。注意：必须设备的特征值支持才可以成功调用，具体参照 characteristic 的 properties 属性"
    },
    {
      ...
      "info": "向低功耗蓝牙设备特征值中写入二进制数据。注意：必须设备的特征值支持才可以成功调用，具体参照 characteristic 的 properties 属性"
    },
    {
      ...
      "info": "启用低功耗蓝牙设备特征值变化时的 notify 功能。注意：必须设备的特征值支持才可以成功调用，具体参照 characteristic 的 properties 属性"
    },
    {
      ...
      "info": "监听低功耗蓝牙连接的错误事件，包括设备丢失，连接异常断开等等。"
    },
    {
      ...
      "info": "监听低功耗蓝牙设备的特征值变化。必须先启用接口才能接收到设备推送的notification。"
    },
    {
    ...
    },
    {
      ...
      "info": "设置是否保持常亮状态。仅在当前小程序生效，离开小程序后设置失效。"
    },
    {
      ...
      "info": "监听用户主动截屏事件，用户使用系统截屏按键截屏时触发此事件"
    },
    {
      ...
      "info": "调用后，用户可以选择将该表单以“新增联系人”或“添加到已有联系人”的方式，写入手机系统通讯录，完成手机通讯录联系人和联系方式的增加。"
    },
    {
      "name": "showToast",
      "value": "showToast({\n\ttitle: ''\n})",
    ...
    },
    {
      ...
      "info": "显示 loading 提示框, 需主动调用 wx.hideLoading 才能关闭提示框"
    },
    {
      "name": "hideToast",
      "value": "hideToast()",
    ...
    },
    {
      ...
      "info": "动态设置置顶栏文字内容，只有当前小程序被置顶时能生效，如果当前小程序没有被置顶，也能调用成功，但是不会立即生效，只有在用户将这个小程序置顶后才换上设置的文字内容。"
    },
    {
    ...
    },
    {
      ...
      "info": "保留当前页面，跳转到应用内的某个页面，使用可以返回到原页面。"
    },
    {
      "name": "redirectTo",
      "value": "redirectTo({\n\turl: ''\n})",
    ...
    },
    {
      ...
      "info": "跳转到 tabBar 页面，并关闭其他所有非 tabBar 页面"
    },
    {
      "name": "navigateBack",
      "value": "navigateBack({\n\t\n})",
      ...
      "info": "关闭当前页面，返回上一页面或多级页面。可通过 ) 获取当前的页面栈，决定需要返回几层。"
    },
    {
      ...
      "info": "创建一个动画实例animation。调用实例的方法来描述动画。最后通过动画实例的方法导出动画数据传递给组件的属性。"
    },
    {
      "name": "pageScrollTo",
      "value": "pageScrollTo({\n\tscrollTop: 0\n})",
    ...
    },
    {
      ...
      "info": "返回一个SelectorQuery对象实例。可以在这个实例上使用等方法选择节点，并使用等方法选择需要查询的信息。"
    },
    {
      ...
      "info": "创建 canvas 绘图上下文（指定 canvasId）"
    },
    {
      ...
      "info": "把当前画布指定区域的内容导出生成指定大小的图片，并返回文件路径。"
    },
    {
      ...
      "info": "开始下拉刷新，调用后触发下拉刷新动画，效果与用户手动下拉刷新一致"
    },
    {
    ...
    },
    {
      ...
      "info": "调用接口获取进而换取用户登录态信息，包括用户的 及本次登录的 。需要依赖会话密钥完成。"
    },
    {
      "name": "checkSession",
      "value": "checkSession({\n\t\n})",
      ...
      "info": "通过上述接口获得的用户登录态拥有一定的时效性。用户越久未使用小程序，用户登录态越有可能失效。反之如果用户一直在使用小程序，则用户登录态一直保持有效。具体时效逻辑由微信维护，对开发者透明。开发者只需要调用wx.checkSession接口。登录态过期后开发者可以再调用wx.login获取新的用户登录态。"
    },
    {
      "name": "authorize",
      "value": "authorize({\n\tscope: ''\n})",
      ...
      "info": "部分接口需要获得同意后才能调用。此类接口调用时，如果用户未授权过，会弹窗询问用户，用户点击同意后方可调用接口。如果用户点了拒绝，则短期内调用不会出现弹窗，而是直接进入 fail 回调。用户可以在小程序设置界面中修改对该小程序的授权信息。本接口用于提前向用户发起授权，调用后会立刻弹窗询问用户是否同意小程序使用某项功能或获取用户的某些数据，但不会实际调用接口。如果用户之前已经同意，则不会出现弹窗，直接返回成功。"
    },
    {
      "name": "getUserInfo",
      "value": "getUserInfo({\n\t\n})",
      ...
      "info": "获取用户信息，withCredentials 为 true 时需要先调用 wx.login 接口。"
    },
    {
      "name": "requestPayment",
    ...
    },
    {
      ...
      "info": "调起用户编辑收货地址原生界面，并在编辑完成后返回用户选择的地址。"
    },
    {
      "name": "addCard",
      "value": "addCard({\n\tcardList: []\n})",
    ...
    },
    {
      ...
      "info": "获取用户过去三十天微信运动步数，需要先调用 wx.login 接口。"
    },
    {
    ...
    },
    {
      ...
      "info": "返回到上一个小程序，只有在当前小程序是被其他小程序打开时可以调用成功"
    },
    {
    ...
    },
    {
      ...
      "info": "将 ArrayBuffer 数据转成 Base64 字符串"
    },
    {
      ...
      "info": "将 Base64 字符串转成 ArrayBuffer 数据"
    }
  ]
}
